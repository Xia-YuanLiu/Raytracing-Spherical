...
class QuadTransferSolver:
    ...
    def trace_b(self, b: float) -> RayResult:
        ...
        u_horizon = self._outer_horizon_u()
        u_stop = u_horizon if u_horizon is not None else self._domain_inner_u()
        u_turn = self._first_turning_point(b, u_start, u_stop)

        if u_turn is None:
            next_region = self._continuation_region(EventType.HORIZON) if u_horizon is not None else None
            if next_region is not None:
                through_result = self._trace_through_horizon(b, u_start, u_horizon, next_region)
            ...
            segment, error = self._make_segment(
                ...
                phi0=0.0,
                direction="inward",
                endpoint_event=EventType.HORIZON if u_horizon is not None else EventType.INNER_BOUNDARY,
        ...
        outbound_phi_end = min(self.options.max_phi, 2.0 * inbound.phi_end)
        outbound_event = EventType.MAX_PHI if outbound_phi_end < 2.0 * inbound.phi_end else EventType.ESCAPE
        outbound_u_end = inbound.u_at(2.0 * inbound.phi_end - outbound_phi_end)
        outbound = RaySegment(
            region=self.metric.region,
            radial_direction="outward",
            phi_start=inbound.phi_end,
            phi_end=outbound_phi_end,
            u_start=u_turn,
            u_end=outbound_u_end,
            endpoint_event=outbound_event,
            evaluator=lambda phi, inbound=inbound: inbound.u_at(2.0 * inbound.phi_end - phi),
        )
        events = [
            RayEvent(EventType.TURNING_POINT, inbound.phi_end, u_turn, self.metric.region),
            RayEvent(outbound_event, outbound.phi_end, outbound.u_end, self.metric.region),
        ]
        diagnostics = RayDiagnostics(
            estimated_error=err_in,
            near_critical=False,
            ...
            max_phi_reached=outbound_event == EventType.MAX_PHI,
            termination_reason=outbound_event.value,
    ...
    def _outer_horizon_u(self) -> float | None:
        horizons = [r for r in self.metric.horizons() if r > 0.0]
        if not horizons:
            return None
        return 1.0 / max(horizons) * (1.0 - self.options.horizon_buffer)

    def _domain_inner_u(self) -> float:
        if self.options.inner_boundary_radius is not None:
            return 1.0 / self.options.inner_boundary_radius
        r_min, _ = self.metric.valid_radial_domain()
        if r_min <= 0.0:
            raise ValueError("metrics without a positive horizon require SolverOptions.inner_boundary_radius")
        return 1.0 / r_min * (1.0 - self.options.horizon_buffer)
    ...
    def _make_segment(
        ...
        def integrate_to(target_u: float) -> tuple[float, float]:
            if endpoint_event == EventType.TURNING_POINT and u1 > u0:
                span = u1 - u0
        ...
        def angle_to_u(phi: float) -> float:
            ...
            return brentq(residual, u0, u1, xtol=self.options.root_atol, rtol=self.options.root_rtol)
...
class OdeTrajectorySolver:
    ...
    def trace_b(self, b: float) -> RayResult:
        ...
        u_horizon = QuadTransferSolver(self.metric, self.observer, self.trace_policy, self.options)._outer_horizon_u()

        def event_horizon(phi: float, y: np.ndarray) -> float:
            if u_horizon is None:
                return 1.0
            return u_horizon - float(y[0])
        ...
        events = [event_turning] if u_horizon is None else [event_horizon, event_turning]
        ...
        horizon_index = 0 if u_horizon is not None else None
        turning_index = 1 if u_horizon is not None else 0
        hit_horizon = horizon_index is not None and len(solution.t_events[horizon_index]) > 0
        hit_turning = len(solution.t_events[turning_index]) > 0
        event_type = EventType.HORIZON if hit_horizon else EventType.TURNING_POINT if hit_turning else EventType.MAX_PHI
        if hit_horizon:
            phi_end = float(solution.t_events[horizon_index][0])
        elif hit_turning:
...
class HamiltonianTrajectorySolver:
    ...
    def trace_b(self, b: float) -> RayResult:
        ...
        r_horizon = max(self.metric.horizons()) if self.metric.horizons() else None
        r_stop = r_horizon * (1.0 + self.options.horizon_buffer) if r_horizon is not None else 0.0
        ...
        def event_horizon(lambda_value: float, y: np.ndarray) -> float:
        ...
        solution = solve_ivp(
            ...
            events=[event_horizon, event_turning],
            rtol=1e-9,
            atol=1e-11,
            dense_output=True,
            max_step=0.5,
        ...
        hit_horizon = len(solution.t_events[0]) > 0
